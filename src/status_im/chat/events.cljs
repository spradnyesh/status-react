...
(defn- mark-messages-seen
  ...
  (let [me                   (:current-public-key db)
        ...
        loaded-message-ids   (keys messages)
        unviewed-messages    (get-in db [:chats chat-id :unviewed-messages])
        unseen-messages-ids  (when (seq unviewed-messages)
                               (filter unviewed-messages loaded-message-ids))]
    (when (seq unseen-messages-ids)
      (handlers-macro/merge-fx cofx
                               {:db (-> db
                                        (assoc-in messages-path
                                                  (reduce (fn [new-messages message-id]
                                                            (assoc-in new-messages [message-id :user-statuses me] :seen))
                                                          messages
                                                          unseen-messages-ids))
                                        (assoc-in [:chats chat-id :unviewed-messages]
                                                  (apply disj unviewed-messages loaded-message-ids)))}
